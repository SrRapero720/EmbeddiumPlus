 ...
 * the License.
 ******************************************************************************/
package me.srrapero720.embeddiumplus.features.TotalDarkness;
...
public class Darkness {
	...
	public static void bake() {
		EmbeddiumPlusConfig.darkNetherFogEffective = EmbeddiumPlusConfig.darkNether.get() ? EmbeddiumPlusConfig.darkNetherFogConfigured.get() : 1.0;
		EmbeddiumPlusConfig.darkEndFogEffective = EmbeddiumPlusConfig.darkEnd.get() ? EmbeddiumPlusConfig.darkEndFogConfigured.get() : 1.0;
	}

	public static boolean blockLightOnly() {
		return EmbeddiumPlusConfig.blockLightOnly.get();
	}

	public static double darkNetherFog() {
		return EmbeddiumPlusConfig.darkNetherFogEffective;
	}

	public static double darkEndFog() {
		return EmbeddiumPlusConfig.darkEndFogEffective;
	}
	...
	private static boolean isDark(Level world) {
		...
		if (dimType == Level.OVERWORLD) {
			return EmbeddiumPlusConfig.darkOverworld.get();
		} else if (dimType == Level.NETHER) {
			return EmbeddiumPlusConfig.darkNether.get();
		} else if (dimType == Level.END) {
			return EmbeddiumPlusConfig.darkEnd.get();
		} else if (world.dimensionType().hasSkyLight()) {
			return EmbeddiumPlusConfig.darkDefault.get();
		} else {
			return EmbeddiumPlusConfig.darkSkyless.get();
		}
	}

	private static float skyFactor(Level world) {
		if (!EmbeddiumPlusConfig.blockLightOnly.get() && isDark(world)) {
			if (world.dimensionType().hasSkyLight()) {
				final float angle = world.getTimeOfDay(0);
				if (angle > 0.25f && angle < 0.75f) {
					final float oldWeight = Math.max(0, (Math.abs(angle - 0.5f) - 0.2f)) * 20;
					final float moon = EmbeddiumPlusConfig.ignoreMoonPhase.get() ? 0 : world.getMoonBrightness();
					final float moonInterpolated = (float) Mth.lerp(moon, EmbeddiumPlusConfig.minimumMoonLevel.get(), EmbeddiumPlusConfig.maximumMoonLevel.get());
	...
	public static void updateLuminance(float tickDelta, Minecraft client, GameRenderer worldRenderer, float prevFlicker) {
		...
		if (world != null) {
			...
			for (int skyIndex = 0; skyIndex < 16; ++skyIndex) {
				...
				float min = Math.max(skyFactor * 0.05f, EmbeddiumPlusConfig.darknessOption.get().value);
				...
				min = Math.max(0.35f * skyFactor, EmbeddiumPlusConfig.darknessOption.get().value);
				...
				for (int blockIndex = 0; blockIndex < 16; ++blockIndex) {
					...
					min = Math.max(0.03f * f, EmbeddiumPlusConfig.darknessOption.get().value);
					red = red * (0.99F - min) + min;
					green = green * (0.99F - min) + min;
					blue = blue * (0.99F - min) + min;
