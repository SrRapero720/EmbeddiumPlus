package vice.templatemod.base.machines;
 ...
 * 1) When constructing the TileEntity, create and store a MachineInventoryZoneContents using createForTileEntity()
 * 2) In your ContainerType<MyContainer>, create a MachineInventoryZoneContents using createForClientSideContainer() and pass it to
...
public class MachineInventoryZone implements IInventory
{
    private MachineInventoryZone(int size) {
        this.ComponentContents = new ItemStackHandler(size);
    }

    private MachineInventoryZone(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda, Notify markDirtyNotificationLambda) {
        this.ComponentContents = new ItemStackHandler(size);
    ...
    private final ItemStackHandler ComponentContents;

    /**
     * Use this constructor to create a MachineInventoryZoneContents which is linked to its parent TileEntity.
     ...
     */
    public static MachineInventoryZone createForTileEntity(int size,
                                                          Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
                                                          Notify markDirtyNotificationLambda) {
        return new MachineInventoryZone(size, canPlayerAccessInventoryLambda, markDirtyNotificationLambda);
    }

    /**
     * Use this constructor to create a MachineInventoryZoneContents which is not linked to any parent TileEntity; i.e.
     ...
     * @return the new ChestContents
     */
    public static MachineInventoryZone createForClientSideContainer(int size) {
        return new MachineInventoryZone(size);
    }
    ...
    public CompoundNBT serializeNBT()  {
        return ComponentContents.serializeNBT();
    }
    ...
    public void deserializeNBT(CompoundNBT nbt)   {
        ComponentContents.deserializeNBT(nbt);
    }
    ...
    public boolean canPlaceItem(int index, ItemStack stack) {
        return ComponentContents.isItemValid(index, stack);
    }
    ...
    public int getContainerSize() {
        return ComponentContents.getSlots();
    }

    @Override
    public boolean isEmpty() {
        for (int i = 0; i < ComponentContents.getSlots(); ++i) {
            if (!ComponentContents.getStackInSlot(i).isEmpty()) return false;
        }
        return true;
    }
    ...
    public ItemStack getItem(int index) {
        return ComponentContents.getStackInSlot(index);
    }

    @Override
    public ItemStack removeItem(int index, int count) {
        if (count < 0) throw new IllegalArgumentException("count should be >= 0:" + count);
        return ComponentContents.extractItem(index, count, false);
    }

    @Override
    public ItemStack removeItemNoUpdate(int index) {
        int maxPossibleItemStackSize = ComponentContents.getSlotLimit(index);
        return ComponentContents.extractItem(index, maxPossibleItemStackSize, false);
    }

    @Override
    public void setItem(int index, ItemStack stack) {
        ComponentContents.setStackInSlot(index, stack);
    }

    @Override
    public void clearContent() {
        for (int i = 0; i < ComponentContents.getSlots(); ++i) {
            ComponentContents.setStackInSlot(i, ItemStack.EMPTY);
        }
    }
    ...
    public ItemStack increaseStackSize(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = ComponentContents.insertItem(index, itemStackToInsert, false);
    ...
    public boolean doesItemStackFit(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = ComponentContents.insertItem(index, itemStackToInsert, true);
