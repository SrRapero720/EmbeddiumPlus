package vice.templatemod.features.machine;
...
import vice.templatemod.base.machines.MachineInventoryZone;

public class MachineContainer extends Container
{

    public static MachineContainer createContainerServerSide(int windowID, PlayerInventory playerInventory,
                                                             MachineInventoryZone inputZoneContents,
                                                             MachineInventoryZone outputZoneContents,
                                                             MachineInventoryZone fuelZoneContents,
                                                             MachineStateData machineStateData) {
        return new MachineContainer(windowID, playerInventory,
                inputZoneContents, outputZoneContents, fuelZoneContents, machineStateData);
    }

    public static MachineContainer createContainerClientSide(int windowID, PlayerInventory playerInventory, net.minecraft.network.PacketBuffer extraData) {
        ...
        MachineInventoryZone inputZoneContents = MachineInventoryZone.createForClientSideContainer(INPUT_SLOTS_COUNT);
        MachineInventoryZone outputZoneContents = MachineInventoryZone.createForClientSideContainer(OUTPUT_SLOTS_COUNT);
        MachineInventoryZone fuelZoneContents = MachineInventoryZone.createForClientSideContainer(FUEL_SLOTS_COUNT);
        MachineStateData machineStateData = new MachineStateData();
    ...
    //  36 - 39 = fuel slots (machineStateData 0 - 3)
    //  40 - 44 = input slots (machineStateData 4 - 8)
    //  45 - 49 = output slots (machineStateData 9 - 13)
    ...
    public MachineContainer(int windowID, PlayerInventory invPlayer,
                            MachineInventoryZone inputZoneContents,
                            MachineInventoryZone outputZoneContents,
                            MachineInventoryZone fuelZoneContents,
                            MachineStateData machineStateData) {
        super(StartupCommon.containerTypeMachineContainer, windowID);
        if (StartupCommon.containerTypeMachineContainer == null)
            throw new IllegalStateException("Must initialise containerTypeMachineContainer before constructing a MachineContainer!");
        this.inputZoneContents = inputZoneContents;
        this.outputZoneContents = outputZoneContents;
        this.fuelZoneContents = fuelZoneContents;
        this.machineStateData = machineStateData;
        this.world = invPlayer.player.world;

        trackIntArray(machineStateData);    // tell vanilla to keep the machineStateData synchronised between client and server Containers
    ...
    public double fractionOfFuelRemaining(int fuelSlot) {
        if (machineStateData.burnTimeInitialValues[fuelSlot] <= 0 ) return 0;
        double fraction = machineStateData.burnTimeRemainings[fuelSlot] / (double)machineStateData.burnTimeInitialValues[fuelSlot];
    ...
    public int secondsOfFuelRemaining(int fuelSlot)	{
        if (machineStateData.burnTimeRemainings[fuelSlot] <= 0 ) return 0;
        return machineStateData.burnTimeRemainings[fuelSlot] / 20; // 20 ticks per second
    }
    ...
    public double fractionOfCookTimeComplete() {
        if (machineStateData.cookTimeForCompletion == 0) return 0;
        double fraction = machineStateData.cookTimeElapsed / (double)machineStateData.cookTimeForCompletion;
        return MathHelper.clamp(fraction, 0.0, 1.0);
    }
    ...
    private MachineInventoryZone inputZoneContents;
    private MachineInventoryZone outputZoneContents;
    private MachineInventoryZone fuelZoneContents;
    private MachineStateData machineStateData;
