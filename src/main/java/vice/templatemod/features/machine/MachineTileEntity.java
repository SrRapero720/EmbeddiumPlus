...
public class TileEntityFurnace extends TileEntity implements INamedContainerProvider, ITickableTileEntity
{
    ...
    private final MachineStateData MachineStateData = new MachineStateData();

    public TileEntityFurnace()
    {
        super(StartupCommon.tileEntityTypeMBE31);

        fuelZoneContents = MachineInventoryZone.createForTileEntity(FUEL_SLOTS_COUNT,
                this::canPlayerAccessInventory, this::setChanged);
        inputZoneContents = MachineInventoryZone.createForTileEntity(INPUT_SLOTS_COUNT,
                this::canPlayerAccessInventory, this::setChanged);
        outputZoneContents = MachineInventoryZone.createForTileEntity(OUTPUT_SLOTS_COUNT,
                this::canPlayerAccessInventory, this::setChanged);
    }
    ...
    public int numberOfBurningFuelSlots()	{
        int burningCount = 0;
        for (int burnTime : MachineStateData.burnTimeRemainings) {
    ...
    public void tick() {
        ...
        if (!ItemStack.isSame(currentlySmeltingItem, currentlySmeltingItemLastTick)) {  // == and != don't work!
        ...
        if (!currentlySmeltingItem.isEmpty()) {
            ...
            if (numberOfFuelBurning > 0) {
                MachineStateData.cookTimeElapsed += numberOfFuelBurning;
            }	else {
                MachineStateData.cookTimeElapsed -= 2;
            }
            if (MachineStateData.cookTimeElapsed < 0) MachineStateData.cookTimeElapsed = 0;

            int cookTimeForCurrentItem = getCookTime(this.level, currentlySmeltingItem);
    ...
    private int burnFuel() {
        ...
        for (int fuelIndex = 0; fuelIndex < FUEL_SLOTS_COUNT; fuelIndex++) {
            if (MachineStateData.burnTimeRemainings[fuelIndex] > 0) {
                --MachineStateData.burnTimeRemainings[fuelIndex];
                ++burningCount;
            }
            ...
            if (MachineStateData.burnTimeRemainings[fuelIndex] == 0) {
                ...
                if (!fuelItemStack.isEmpty() && getItemBurnTime(this.world, fuelItemStack) > 0) {
                    ...
                    MachineStateData.burnTimeRemainings[fuelIndex] = burnTimeForItem;
                    MachineStateData.burnTimeInitialValues[fuelIndex] = burnTimeForItem;
     ...
     * @param MachineInventoryZone
     * @param slotIndex
     * @param itemStackOrigin
     * @return true if the given ItemStack will fit completely; false otherwise
     */
    public boolean willItemStackFit(MachineInventoryZone MachineInventoryZone, int slotIndex, ItemStack itemStackOrigin) {
        ItemStack itemStackDestination = MachineInventoryZone.getStackInSlot(slotIndex);
        ...
        if (sizeAfterMerge <= MachineInventoryZone.getInventoryStackLimit() && sizeAfterMerge <= itemStackDestination.getMaxStackSize()) {
            return true;
        }
        return false;
    }
    ...
    public CompoundNBT write(CompoundNBT parentNBTTagCompound)
    {
        ...
        MachineStateData.putIntoNBT(parentNBTTagCompound);
    ...
    public void read(BlockState blockState, CompoundNBT nbtTagCompound)
    {
        ...
        MachineStateData.readFromNBT(nbtTagCompound);
